...
async function fetchColumns(boardId: string): Promise<ColumnWithCards[]> {
  ...
  const { data, error } = await supabase
    .from('columns')
    .select('id, board_id, title, position, created_at, updated_at, archived_at, cards(id, column_id, board_id, title, description, status, position, assignee_id, due_date, created_at, updated_at, archived_at, archived_with_column)')
    .eq('board_id', boardId)
    .is('archived_at', null)
